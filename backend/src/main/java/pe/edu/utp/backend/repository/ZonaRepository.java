...
public interface ZonaRepository extends JpaRepository<Zona, Long> {
    ...
    @Query("SELECT z FROM Zona z WHERE z.lugar.id = :idLugar")
    List<Zona> buscarZonasPorLugar(@Param("idLugar") Long idLugar);

    @Query("SELECT COUNT(z) > 0 FROM Zona z WHERE LOWER(z.nombre_zona) = LOWER(:nombreZona) AND z.lugar.id = :idLugar")
